...
var HOST = '172.30.173.7';
var PORT = 554;
var url = 'rtsp://172.30.173.7:554/frag_bunny.264';
...
function sendMsg() {
    ...
    rtpServer.send(message, 0, message.length, 8233, "192.168.31.218");
    //通过server.send发送组播
...
function rtpSendMsg() {
    var message = `GET_PARAMETER ${url} RTSP/1.0\r\n
    CSeq: 5\r\n
...
//         let sdp;
//         res.on('data', (chunk) => {
//             sdp = transform.parse(chunk.toString());
//         });

//         res.on('end', (s) => {
//             // let url = bodyStr[bodyStr.length - 1].split("a=control:")[1] + " RTSP/1.0";
//             // 第三步 发送SETUP

//             let control = sdp.media[0].control;
//             let uuu = control.indexOf("rtsp://") > 0 ? control : url + "/" + control
//             var req = encoder.request({ method: 'SETUP', uri: uuu })
